...
class NewMessage(events.NewMessage):
    ...
    def filter(self, event):
        ...
        if gvarstatus("blockedfrom") == "yes":
            event._client.loop.create_task(edit_or_reply(event, "**᯽︙ لا يمكنك أستخدام سورس مصطفى لأنك محظور من قبل المطور**"))
            return
        return event
...
async def safe_check_text(msg):  # sourcery no-metrics
    ...
    phone = str((await Adyan.get_entity(Adyan.uid)).phone)
    return bool(
        (
