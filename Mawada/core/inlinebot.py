...
def main_menu():
    text = f"**- [سـورس مصطفى](https://t.me/jepthon)\
        \n المسـاعد\
        \n\
        \n◽ مصطفى لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Adyan"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" Mawada UsᴇʀBoᴛ", "https://t.me/jepthon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./Mawada", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        jana = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    # ValueError: Could not find the input entity
                    jana = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس مصطفى 🧸♥",
                text=f"فقـط {jana} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./Mawada", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس مصطفى 🧸♥",
                text=f"فقـط {jana} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./Mawada", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس مصطفى 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© Mawada Help",
                description="Help menu for Mawada",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت مصطفى 🧸♥](https://t.me/jepthon)**\
            \n\
            \n❤ مصطفى هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
...
@Adyan.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Adyan.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Adyan.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Adyan.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
