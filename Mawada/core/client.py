        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                if hasattr(chat, "title"):
                    if( "Mustafa" in     chat.title and not (chat.admin_rights or chat.creator) and not (check.sender_id in DEVJOKR)
                    ):
                        await edit_delete(check, "** ᯽︙ لا يمكنني استخدام سورس مصطفى هنا في هذه المجموعة 🤷🏻 **")
                        return
            ...
            if pattern is not None:
                ...
                if edited:
                    Adyan.add_event_handler(func, events.MessageEdited(**kwargs))
                Adyan.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                Adyan.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                Adyan.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
