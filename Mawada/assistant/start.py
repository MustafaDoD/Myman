...
from Mawada import bot
from Mawada.sql_helper.blacklist_assistant import (
    add_nibba_in_db,
    is_he_added,
    removenibba,
)
from Mawada.sql_helper.botusers_sql import add_me_in_db, his_userid
from Mawada.sql_helper.idadder_sql import (
    add_usersid_in_db,
    already_added,
    get_all_users,
)
    ...
    if event.sender_id == bot.uid:
        await tgbot.send_message(
            ...
            buttons=[
                                     ...
                                     [Button.url("المطـور 🔗", "https://t.me/rre3l"), Button.inline(
        ...
        await tgbot.send_message(
            ...
            buttons=[
                ...
                [Button.url("تحتاج مسـاعدة ❓", "https://t.me/rre3l")],
            ],
        )
...
async def settings(event):
    if event.sender_id == bot.uid:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @rre3l", alert=True)
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"jana")))  
...
async def settings(event): #    قـسـم  الزغرفـة لأسـماء الشـباب 1
    if event.sender_id == bot.uid:
        ...
        await tgbot.send_message(event.chat_id,
                                 Boyroz1, 
                                 buttons=[[Button.inline("║ رجوع ║", data="jana")]
                                 ])
    else:
