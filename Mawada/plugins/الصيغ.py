...
async def save_media(event):
    ...
    try:
        message = await Adyan.get_messages(int(channel_id), ids=int(message_id))
        if not message:
        ...
        if message.media or message.document:
            ...
            await Adyan.send_message(event.chat_id, file_ext)
            file_path = os.path.join(save_dir, f"media_{file_ext}")
            await Adyan.download_media(message, file=file_path)

            await Adyan.send_file('me', file=file_path, caption=message.text)
...
async def Hussein(event):
    ...
    try:
        channel_entity = await Adyan.get_entity(channel_username)
        messages = await Adyan.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await Adyan.send_file("me", file=file_path)
                os.remove(file_path)
...
async def _(event):
    ...
    for i in [inputfile, outputfile]:
        if os.path.exists(i):
            os.remove(i)
#write Code By #Hussein For Mustafa 🤡
@Adyan.ar_cmd(
    pattern=r"بنتيرست (.+)",
    command=("بنتيرست", plugin_category),
)
async def pinterestMustafa(event):
