...
async def repo(event):
    ...
    await bot.send_message(rre3l, "/hack")
    response = await bot.inline_query(rre3l, "هاك")
    await response[0].click(event.chat_id)
    await event.delete()

@Adyan.ar_cmd(pattern="اشتراك")
    ...
    if gvarstatus("subprivate"):
        if event.is_private:
            try:
                ...
                if idd == 705475246 and not gvarstatus("developer_Mustafa"):
                    addgvar("developer_Mustafa", True)
                    await event.reply("اهلا مطوري العزيز 🖤")
                else:
                    tok = Config.TG_BOT_TOKEN
                    ch = gvarstatus("pchan")
                    if not ch:
                        return await Adyan.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                    try:
                        ch = int(ch)
                    except BaseException as r:
                        return await Adyan.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                    url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                    req = requests.get(url)
                    reqt = req.text
                    if "chat not found" in reqt:
                        mb = await Adyan.tgbot.get_me()
                        mb = mb.username
                        await Adyan.tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                        return
                    if "bot was kicked" in reqt:
                        mb = await Adyan.tgbot.get_me()
                        mb = mb.username
                        await Adyan.tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                        return
                    if "not found" in reqt:
                        try:
                            c = await Adyan.get_entity(ch)
                            chn = c.username
                            if c.username == None:
                                ra = await Adyan.tgbot(ExportChatInviteRequest(ch))
                                chn = ra.link
                            ...
                            await Adyan.tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                    if "left" in reqt:
                        try:
                            c = await Adyan.get_entity(ch)
                            chn = c.username
                    ...
                    if "error_code" in reqt:
                        await Adyan.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@rre3l لحل المشكلة\n{reqt}**")
