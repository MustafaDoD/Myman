...
async def ban_user(chat_id, i, rights):
    try:
        await Adyan(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        

@Adyan.ar_cmd(pattern="ارسل")
...
@Adyan.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await Adyan(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in Adyan.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await Adyan.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@Adyan.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@Adyan.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@Adyan.ar_cmd(pattern="الغاء التفليش")
...
@Adyan.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await Adyan.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await Adyan.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await Adyan(LeaveChannelRequest(channel_username))
...
@Adyan.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@Adyan.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@Adyan.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
# الكود من كتابة فريق مصطفى بس تسرقة تنشر بقناة الفضايح انتَ وقناتك 🖤
@Adyan.ar_cmd(pattern=r"ذكاء(.*)")
async def hussein(event):
...
is_Reham = False
No_group_Joker = "@rre3lsupport"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_Mustafa = []

@Adyan.ar_cmd(pattern=r"الذكاء تفعيل")
async def enable_bot(event):
    global is_Reham
    if not is_Reham:
        is_Reham = True
        active_Mustafa.append(event.chat_id)
...
@Adyan.ar_cmd(pattern=r"الذكاء تعطيل")
async def disable_bot(event):
    global is_Reham
    if is_Reham:
        is_Reham = False
        active_Mustafa.remove(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_Mustafa:
        return
    message = event.message
...
async def Hussein(event):
    ...
    if sender_id != 705475246:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            Mustafa_entity = await Adyan.get_entity(sender.id)
            Mustafa_profile = f"[{Mustafa_entity.first_name}](tg://user?id={Mustafa_entity.id})"
            await event.reply(f"**᯽︙ عذرًا {Mustafa_profile}، يُرجى عدم إرسال الرسائل التي تحتوي على إيموجي المُميز**")
@Adyan.ar_cmd(pattern="المميز تفعيل")
...
@Adyan.ar_cmd(pattern="المميز تعطيل")
async def disable_emoji_blocker(event):
    global Ya_Hussein
    Ya_Hussein = False
    active_joker.remove(event.chat_id)
    await event.edit("**᯽︙ تم تعطيل امر منع الايموجي المُميز بنجاح ✓ **")
remove_admins_Mustafa = {}
#الكود تمت كتابته من قبل مطورين مصطفى اذا الك نية تخمطه اذكر حقوق السورس @rre3l
@Adyan.on(events.ChatAction)
async def Hussein(event):
    if gvarstatus("Mn3_Kick"):
        if event.user_kicked:
            ...
            if chat and user_id:
                now = datetime.now()
                if user_id in remove_admins_Mustafa:
                    if (now - remove_admins_Mustafa[user_id]).seconds < 60:
                    ...
                    remove_admins_Mustafa.pop(user_id)
                    remove_admins_Mustafa[user_id] = now
                else:
                    remove_admins_Mustafa[user_id] = now

@Adyan.ar_cmd(pattern="منع_التفليش", require_admin=True)
async def Hussein_Mustafa(event):
    addgvar("Mn3_Kick", True)
    await event.edit("**᯽︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")

@Adyan.ar_cmd(pattern="سماح_التفليش", require_admin=True)
async def Hussein_Mustafa(event):
    delgvar("Mn3_Kick")
    await event.edit("**᯽︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")
message_counts = {}
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            await Adyan.edit_permissions(event.chat_id, user_id, send_messages=False)
            sender = await event.get_sender()
            Mustafa_entity = await Adyan.get_entity(sender.id)
            Mustafa_profile = f"[{Mustafa_entity.first_name}](tg://user?id={Mustafa_entity.id})"
            explanation_message = f"**᯽︙ تم تقييد {Mustafa_profile} من إرسال الرسائل بسبب تفعيله نشر التلقائي**"
...
async def handle_messages(event):
    if gvarstatus("5a9_dis"):
        sender_id = event.sender_id
        current_user_id = await Adyan.get_me()
        if event.is_private and sender_id != current_user_id.id:
            await event.delete()
            if sender_id not in Mustafa_Menu:
                Mustafa_time = Mustafa_waqt()
                Mustafa_message = gvarstatus("Mustafa_message") or f"صاحب الحساب قافل خاصة قبل يلا دعبل"
                Mustafa_url = gvarstatus("Mustafa_url") or "https://telegra.ph/file/ee30cda28bd1346e54cb3.jpg"
                await Adyan.send_file(sender_id, Mustafa_url, caption=f'**{Mustafa_message}**\n**مدة الغياب: {Mustafa_time}**')
                Mustafa_Menu.add(sender_id)
@Adyan.ar_cmd(pattern="الخاص تعطيل")
...
@Adyan.ar_cmd(pattern="الخاص تفعيل")
async def joker5a9(event: Message):
    global afk_start_time
    delgvar("5a9_dis")
    afk_start_time = None
...
async def get_bot_entity():
    return await Adyan.get_entity('me')

@Adyan.on(events.NewMessage(outgoing=True, pattern=r'\.اسرع (.*)'))
    ...
    if is_game_started and not is_word_sent and word.lower() in event.raw_text.lower():
        if event.chat_id:
            ...
            if bot_entity and event.sender_id != bot_entity.id:
                ...
                points_text = '\n'.join([f'{i+1}• {(await Adyan.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                await Adyan.send_message(event.chat_id, f'الف مبرووووك 🎉 الاعب ( {sender_first_name} ) فاز! \n اصبحت نقاطة: {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
joker = [
    "تلعب وخوش تلعب 👏🏻",
    "لك عاش يابطل استمر 💪🏻",
...
@Adyan.on(events.NewMessage(pattern=r'\طك (\d+)'))
async def handle_strike(event):
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if strike_position == correct_answer:
            ...
            rre3l = random.choice(joker)
            await event.reply(f"**{rre3l}**\n{format_board(game_board, numbers_board)}")

@Adyan.on(events.NewMessage(pattern=r'\جيب (\d+)'))
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if 1 <= guess <= 6:
            if guess == correct_answer:
                ...
                points_text = '\n'.join([f'{i+1}• {(await Adyan.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
                await Adyan.send_message(event.chat_id, f'الف مبروووك 🎉 الاعب ( {sender_first_name} ) وجد المحبس 💍!\n{format_board(game_board, numbers_board)}')
                game_board = [row[:] for row in original_game_board]
                await Adyan.send_message(event.chat_id, f'نقاط الاعب : {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
            else:
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
...
@Adyan.on(events.NewMessage(pattern=r'\انا'))
