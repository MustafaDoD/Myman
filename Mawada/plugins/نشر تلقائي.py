...
@Adyan.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        Mawada = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        Mawada = str(jok).replace("-100", "")
    else:
        try:
            Mawada = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await Adyan.send_message(int(chat), event.message)
