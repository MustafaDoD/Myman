...
# كتابة فريق مصطفى المتغيرات تثبت ودي
# تخمط اذكر حقوق غيرها انت فرخ و دي 😂


@Adyan.on(admin_cmd(pattern=r"ضيف ?(.*)"))
async def get_users(event):   
    ...
    Mawada = await get_chatinfo(event) ; chat = await event.get_chat()
    if event.is_private:
              return await roz.edit("**▾∮ لا يمكننـي اضافـة المـستخدمين هـنا**")    
    s = 0 ; f = 0 ; error = 'None'   
  
    await roz.edit("**▾∮ حـالة الأضافة:**\n\n**▾∮ تتـم جـمع معـلومات الـمستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(Mawada.full_chat.id):
                try:
...
#تم كتابة الكود من قبل مطورين مصطفى ممنوع السرقة اخي الكريم
@Adyan.on(admin_cmd(pattern=r"اضافة_جهاتي ?(.*)"))
