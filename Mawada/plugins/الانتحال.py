# Copyright (C) 2021 Mawada TEAM
# FILES WRITTEN BY  @rre3l
import html
import base64
...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @rre3l"
)

@Adyan.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await Adyan.get_me()
...
#Reda
jeps = ["Mawada", "HuResupport"]
@Adyan.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await Adyan(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await Adyan(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await Adyan(functions.channels.EditTitleRequest(
                channel=await Adyan.get_entity(event.chat_id),
        ...
        try:
            await Adyan(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await Adyan.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await Adyan.upload_file(profile_pic)
        try:
            await Adyan(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @rre3l")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
#Reda
@Adyan.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in Adyan.iter_profile_photos(event.chat_id, limit=1) :
                    await Adyan(
