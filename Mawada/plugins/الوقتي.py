...
from Mawada import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, Adyan, logging
...
autopic_path = os.path.join(os.getcwd(), "Mawada", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "Mawada", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "Mawada", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "Mawada", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "Mawada", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "Mawada/helpers/styles/PaybAck.ttf"
...
async def Mustafa_timezone(event, tz_name):
            ...
            if i > 0:
                await Adyan(
                    functions.photos.DeletePhotosRequest(
                        await Adyan.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            result = await Adyan(functions.photos.UploadProfilePhotoRequest(file=await Adyan.upload_file(autophoto_path)))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await Adyan.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in Adyan.iter_profile_photos(int(dgp), limit=1) :
                    await Adyan(
            ...
            i += 1
            await Adyan(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await Adyan.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        ...
        try:
            await Adyan(functions.channels.EditTitleRequest(
                channel=await Adyan.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await Adyan.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
    ...
    while AUTONAMESTART:
        ...
        for normal in HM:
            if normal in normzltext:
                ...
                namefont = namerzfont[normzltext.index(normal)]
                HM = HM.replace(normal, namefont)
                rre3l = gvarstatus("TIME_JEP") or ""
        name = f"{rre3l} {HM}"
        LOGS.info(name)
        try:
            await Adyan(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await Adyan(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@Adyan.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Adyan.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@Adyan.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@Adyan.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Adyan.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
Adyan.loop.create_task(digitalpicloop())
Adyan.loop.create_task(digitalgrouppicloop())
Adyan.loop.create_task(autoname_loop())
Adyan.loop.create_task(autobio_loop())
Adyan.loop.create_task(group_loop())