...
from Mawada import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Mawada/plugins/{shortname}.py")
        checkplugins(path)
        name = "Mawada.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"Mawada/plugins/{shortname}.py")
            name = f"Mawada.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Mawada.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def remove_plugin(shortname):
    ...
    try:
        name = f"Mawada.plugins.{shortname}"
        for i in reversed(range(len(Adyan._event_builders))):
            ev, cb = Adyan._event_builders[i]
            if cb.__module__ == name:
                del Adyan._event_builders[i]
    except BaseException:
        raise ValueError
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Mawada/plugins/assistant/{shortname}.py")
        name = "Mawada.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Mawada.plugins.assistant" + shortname] = mod
